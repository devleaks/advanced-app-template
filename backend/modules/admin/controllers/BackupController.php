...
class BackupController extends Controller
{
    ...
    public function actionCreate()
    {
		...
		if($model->doBackup() && $model->save()) {
			Yii::$app->session->setFlash('success', Yii::t('app', 'Backup completed.'));
			return $this->redirect(['index']);
		}

		Yii::$app->session->setFlash('error', Yii::t('app', 'There was an error producing the backup: {0}.', VarDumper::dumpAsString($model->errors, 4, true))); 			
		return $this->redirect(['index']);
    }
	...
	public function actionRestoreDev($id) {
		...
		if(Backup::restore($fn))
			Yii::$app->session->setFlash('success', Yii::t('app', 'Backup restored.'));
		else
			Yii::$app->session->setFlash('danger', Yii::t('app', 'Backup not restored.'));
